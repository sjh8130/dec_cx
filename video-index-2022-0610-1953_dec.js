...
function showHTML5Player(paras) {
	Ext.get('reader').setHTML('<video id=\x22video\x22 class=\x22video-js vjs-default-skin vjs-big-play-centered\x22></video>'),
		new ans.VideoJs({
			'videojs': 'video',
			'params': paras
		}),
		...
		Ext.Ajax.request({
				...
				switch (oData.status) {
					case 'success':
						...
						if (setting && setting.control) {
							...
							for (var i = 0; i < attachments.length; i++) {
								...
								if (spec == s) {
									Ext.apply(paras, setting.defaults),
										paras.headOffset = m.headOffset ? Math.floor(parseInt(m.headOffset) / 0x3e8) : 0,
									...
									m.property && m.property.fastforward && m.property.fastforward == 'true' && (paras.enableFastForward = 0);
									m.property && typeof m.property.doublespeed != 'undefined' && m.property.doublespeed == 0 && (paras.doublespeed = 0,
										doublespeed = 0);
							...
							var schoolDoubleSpeed = paras.schooldoublespeed;
							typeof schoolDoubleSpeed == 'undefined' || schoolDoubleSpeed == 2 ? paras.doublespeed = doublespeed : paras.doublespeed = schoolDoubleSpeed;
						}
						paras.ignoreVideoCtrl == 1 && (paras.enableFastForward = 1,
							paras.enableSwitchWindow = 1);
			...
			'failure': function (resp) {
				resp.status == 0 ? (timer && clearInterval(timer),
					note1Wrap.update('<div class=\x22line1\x22><div class=\x22line2\x22></div></div><p class=\x22tipStyle\x22>视频加载超时，请刷新页面重试【9003】</p>'),
	...
	var videoMp4Support = function () {
		...
		if (typeof elem.canPlayType == 'function') {
			...
			if (playable.toLowerCase() == 'maybe' || playable.toLowerCase() == 'probably')
				return true;
			var playable = elem.canPlayType('video/mp4;codecs=\x22avc1.64001E,mp4a.40.2\x22');
			if (playable.toLowerCase() == 'maybe' || playable.toLowerCase() == 'probably')
				return true;
		}
		return false;
	...
	try {
		...
		if (dtype == 'ZT' || dtype == 'HB' || dtype == 'MAG') {
			var search = parent.location.search || ''
				...
				, chapterId = setting.defaults.knowledgeid
				, duration = window.duration
				, _from_ = GetQueryString('_from_')
				...
				for (var i = 0, len = attachments.length; i < len; i++) {
					...
					if (obj == objectId) {
						if (st == 0)
							...
							st == 1 ? enc = item.enc1 : enc = item.enc2;
						break;
					}
				}
			var logUrl = location.protocol + '//data-xxt.aichaoxing.com/analysis/datalog?s=' + courseId + '&c=' + chapterId + '&o=' + objectId + '&st=' + st + '&m=' + type + '&d=' + duration * 0x3e8 + '&u=' + uid + '&_from_=' + _from_ + '&enc=' + enc
				, logRequest = function () {
					Ext.Ajax.request({
						'url': logUrl,
						'success': function (response) { }
			...
			if (st == 1)
				...
				st == 2 && (playLogTimer && clearInterval(playLogTimer));
		}
	} catch (error) { }
}
				...
				if (list[i] == '@感动@')
					...
					if (list[i] == '@疑惑@')
						...
						if (list[i] == '@献花@')
							...
							if (list[i] == '@鼓掌@')
								content += '<span><i class=\x22clapIcon\x22></i>鼓掌</span>';
							else
...
function checkJobCountLimit(AAAA, BBBB) {
	/\/studentstudy/.test(top.location.pathname) && (BBBB == 0 ? top.showJobLimitTip() : top.showVideoTimeLimitTip(),
...
function startFaceCollection(player, collectionType, obj) {
	if (/\/studentstudy/.test(top.location.pathname)) {
		...
		len > 0 && ($('#videoJobId', top.document).val(videoJobId),
	...
	window.setInterval('reSizeIframe()', 0xc8),
		...
		$('.writeNote').click(function () {
			...
			var cPlayer = videojs('video')
				...
				, minuteTime = parseInt(videoTime / 0x3c);
