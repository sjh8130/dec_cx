...
Ext.onReady(function () {
	window.setInterval('reSizeIframe()', 0xc8),
		...
		$('.writeNote').click(function () {
			...
			var cPlayer = videojs('video'), videoTime = parseInt(cPlayer.currentTime()), videoTime = parseInt(videoTime / 0x3c);
			videoTime < 0xa && (videoTime = '0' + videoTime.toString());
			var secondTime = parseInt(videoTime % 0x3c);
			secondTime < 0xa && (secondTime = '0' + secondTime.toString()),
				$('.Note_name', parent.parent.document).html(videoName + ' ' + videoTime + ':' + secondTime),
