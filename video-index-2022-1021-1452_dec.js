...
function showHTML5Player(paras) {
	...
	if (typeof aplus_queue != 'undefined') {
		var var_20221021_1 = parent.AttachmentSetting;
		if (var_20221021_1) {
			var var_20221021_2 = var_20221021_1.aplus_video_id, var_20221021_3 = var_20221021_1.aplus_resource_id, var_20221021_4 = var_20221021_1.knowledgename, var_20221021_5 = var_20221021_1.coursename, var_20221021_6 = {
				'video_name': var_20221021_4,
				'video_id': var_20221021_2,
				'resource_id': var_20221021_3,
				'resource_name': var_20221021_5
			};
			$('.vjs-big-play-button')[0x0].addEventListener('click', function () {
				var var_20221021_7 = true;
				typeof aplus_queue != 'undefined' && aplus_queue.push({
					'action': 'aplus.record',
					'arguments': ['video_pybk_click', 'CLK', var_20221021_6]
				});
			}),
				$('.vjs-mute-control')[0x0].addEventListener('click', function () {
					!$(this).hasClass('vjs-vol-0') ? typeof aplus_queue != 'undefined' && aplus_queue.push({
						'action': 'aplus.record',
						'arguments': ['no_sound_click', 'CLK', var_20221021_6]
					}) : typeof aplus_queue != 'undefined' && aplus_queue.push({
						'action': 'aplus.record',
						'arguments': ['enable_sound_click', 'CLK', var_20221021_6]
					});
				}),
				$('.vjs-volume-level')[0x0].addEventListener('mouseup', function () {
					typeof aplus_queue != 'undefined' && aplus_queue.push({
						'action': 'aplus.record',
						'arguments': ['volume_control_drag', 'OTHER', var_20221021_6]
					});
				}),
				$('.vjs-fullscreen-control')[0x0].addEventListener('click', function () {
					$(this).attr('title') == '全屏' ? typeof aplus_queue != 'undefined' && aplus_queue.push({
						'action': 'aplus.record',
						'arguments': ['full_screen_click', 'CLK', var_20221021_6]
					}) : typeof aplus_queue != 'undefined' && aplus_queue.push({
						'action': 'aplus.record',
						'arguments': ['small_screen_click', 'CLK', var_20221021_6]
					});
				});
		}
	}
...
function loadVideo() {
	...
	mid = config('mid');
	var percent = 0x0, vbegin = config('vbegin'), vend = config('vend'), jobid = config('jobid') || '', danmaku = data ? data.danmaku == null ? 0x0 : data.danmaku : 0x0, videoTopicCloud = setting && setting.videoTopicCloud, var_20221021_8 = setting && setting.supportHeartbeat, var_20221021_9 = setting && setting.requestConfigUrl, fastforward = config('fastforward') == 'true' ? true : false, switchwindow = config('switchwindow') == 'true' ? true : false, note = Ext.get('note'), hl = Ext.get('hl'), note1Wrap = Ext.get('note1-wrap'), note1 = Ext.get('note1'), timer = null, rt = data ? data.rt ? data.rt : 0.9 : 0.9;
	...
	function setTopicName(list) {
		...
		if (Ext.select('.zsCloud_ul') != null) {
			...
			for (var i = 0x0, len = list.length; i < len; i++) {
				spanList += '<li><span class=\x27zsCloud_span topicId' + list[i].id + "' onclick='markersPlayer(this)'>" + list[i].name + ' </span></li>';
			}
			...
			'success': function (response) {
				...
				switch (oData.status) {
					case 'success':
						...
						if (setting && setting.control) {
							...
							for (var i = 0x0; i < attachments.length; i++) {
								...
								if (spec == s) {
									...
									try {
										if (var_20221021_8 && top.location.href.indexOf('/studentstudy') > -0x1) {
											var var_20221021_10 = paras.courseid || 0x0, var_20221021_11 = paras.clazzId || 0x0, var_20221021_12 = paras.knowledgeid || 0x0, var_20221021_13 = paras.cookieFid || 0x0, var_20221021_14 = paras.headOffset || 0x0, var_20221021_15 = paras.userid || 0x0;
											Ext.Ajax.request({
												'url': var_20221021_9,
												'async': false,
												'params': {
													'uid': var_20221021_15,
													'schoolId': var_20221021_13,
													'moocClassId': var_20221021_11,
													'moocCourseId': var_20221021_10,
													'chapterId': var_20221021_12,
													'objectId': objectid,
													'videoProgress': var_20221021_14,
													'totalVideoDuration': var_20220922_4,
													'enc': paras.videoEnc
												},
												'method': 'get',
												'success': function (var_20221021_16) {
													var var_20221021_17 = eval('(' + var_20221021_16.responseText + ')');
													var_20221021_17.result === 0x1 ? (paras.eventArray = var_20221021_17.eventArray,
														paras.supportHeartbeat = true) : paras.supportHeartbeat = false;
												},
												'failure': function () {
													paras.supportHeartbeat = false;
												}
											});
										}
									} catch (e) {
						...
						(top.location.href.indexOf('nodedetailcontroller/visitnodedetail') > -0x1 || top.location.href.indexOf('mycourse/teacherstudy') > -0x1) && parseInt(var_20220922_3) === 0x2 && (var_20220922_13 = '<span style = "display: inline-block;width: 40px; height: 20px;font-size: 12px;background:#F9F9FA;border:1px solid #D5E2F1;' + 'border-radius:4px;text-align:center;line-height:20px;color: #6B89B3;position: absolute;right: 0; top: 0px;">动画</span>',
							var_20220922_14 = '<span style ="display: inline-block;overflow: hidden;float: right;text-overflow: ellipsis;white-space: nowrap;width: 40px;' + 'height: 20px;font-size: 12px;background: #F9F9FA;border:1px solid #D5E2F1;border-radius:4px;text-align: center;line-height:20px;color:#6B89B3;right:0;top:0;\x22>动画</span>');
						if (isVideoVisibleName != 'false')
							Ext.fly(window.frameElement).parent().first('.ans-job-icon') != null ? top.location.href.indexOf('nodedetailcontroller/visitnodedetail') > -0x1 ? Ext.fly(window.frameElement).parent().insertHtml('afterBegin', '<div title="' + videoName + '\x22 style=\x22width: 500px;display: inline-block;line-height: 23px;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;\x22>' + videoName + '</div>' + var_20220922_14) : (Ext.fly(window.frameElement).parent().first('.ans-job-icon').insertHtml('beforeEnd', '<span title="' + videoName + '\x22 style=\x22width: 500px;margin-left: 120px;display: inline-block;height: 23px;line-height: 23px;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;\x22>' + videoName + '</span>' + var_20220922_13),
								var_20220922_13 != '' && Ext.fly(window.frameElement).parent().addCls('relativeClass')) : (Ext.fly(window.frameElement).parent().insertHtml('afterBegin', '<div title="' + videoName + '" style="width: 500px;display: inline-block;line-height: 23px;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;">' + videoName + '</div>' + var_20220922_14),
						...
						if (supportH5Video())
							data.danmaku && (loadMultiFile([{
								...
								typeof aplus_queue != 'undefined' && (paras.aplus_video_id = setting.aplus_video_id,
									paras.aplus_resource_id = setting.aplus_resource_id,
									paras.knowledgename = setting.knowledgename,
									paras.coursename = setting.coursename),
...
function showPopIframe(var_20221021_18, var_20221021_19) {
	try {
		top.showCustomPop && (var_20221021_19.pause(),
			top.showCustomPop(var_20221021_18),
			popPlayState = setInterval(function () {
				top.popPlayState && (top.popPlayState = false,
					clearPopStateInterval(),
					var_20221021_19.eventCount <= 0x0 && var_20221021_19.play());
			}, 0x3e8));
	} catch (e) {
		console.log(e);
	}
